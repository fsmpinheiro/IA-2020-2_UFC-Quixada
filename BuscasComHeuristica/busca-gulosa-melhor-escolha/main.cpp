...
int main(){
    ...
    if(!cidadesTXT){
        ...
        if ( _m != 0 ) {
            while ( _m != 0 && cidadesTXT >> str_cidade >> _m) {
                City * _cidade = new City( iterador, str_cidade, 0 );
                ...
                iterador++;
                                                                                ...
                                                                                //cout << "Tamanho do vetor: " << mapaDaRomenia.sizeOfVectorOfCyties() << "\n\n";
            }
    ...
    cout << mapOfRomain.getCityByName( "Fortaleza" )->getNome() << "\n\n";
            ...
            cout << "Arquivo mapa.txt que descreve as transições no mapa não encontrado." <<endl<<endl<<endl;
                ...
                if( _n != 0){
                    while ( _n != 0 && mapaTXT >> _origem >> _destino >> _distancia >> _n) {
                        TransitionTo _tempTransition = *new TransitionTo( _origem.append("-").append(_destino), mapOfRomain.getCityByName( _destino ) , _distancia );
                        //_tempTransition.setDestino(  _distancia );
